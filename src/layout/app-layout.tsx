      ...
      <HStack spacing={4}>
        ...
        {hasAccess(RoqResourceEnum.CONVERSATION, AccessOperationEnum.READ, AccessServiceEnum.PLATFORM) && (
          <ChatMessageBell
            ...
            icon={<ChatIcon color="primary.main" width="20px" height="20px" />}
          />
        )}

        <NotificationBell
          className="layout-notification-bell"
          icon={<NotificationIcon color="primary.main" width="16px" height="20px" />}
        />
        <Flex alignItems={'center'}>
          <Box>{session?.roqUserId && <UserAccountDropdown className="layout-user-profile" />}</Box>
        </Flex>
      </HStack>
    </Flex>
  );
